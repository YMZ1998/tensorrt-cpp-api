...
TRT_CPP_API_EXPORT Result<DType> dtypeOfCvDepth(int cvDepth);
...
TRT_CPP_API_EXPORT Result<TensorView> viewOf(const cv::cuda::GpuMat &mat, Layout layout = Layout::kNHWC);

/// Host view over a continuous cv::Mat as an HWC tensor [rows, cols, channels].
TRT_CPP_API_EXPORT Result<TensorView> viewOf(const cv::Mat &mat, Layout layout = Layout::kNHWC);

/// Upload a cv::Mat to a freshly allocated device Tensor (async on `stream`).
TRT_CPP_API_EXPORT Result<Tensor> upload(const cv::Mat &mat, const Stream &stream);
